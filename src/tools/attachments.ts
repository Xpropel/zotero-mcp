...
export function registerAttachmentTools(server: McpServer): void {
  server.tool(
    "zotero_manage_attachments",
    "List, import, link, update, or delete attachments for a Zotero item. " +
      "Requires the Zotero MCP Local Bridge plugin for write actions.",
    {
      action: z.enum(["list", "import_file", "link_file", "link_url", "update", "delete"]).describe("Action to perform"),
      parent_item_key: z.string().optional().describe("Parent item key for list/import/link actions"),
      attachment_key: z.string().optional().describe("Attachment item key for update/delete actions"),
      file_path: z.string().optional().describe("Local file path for import_file or link_file"),
      ...
      tags: z.array(z.string()).optional().describe("Replace attachment tags on update"),
    ...
    async ({ action, parent_item_key, attachment_key, file_path, url, title, content_type, tags, confirm, permanent }) => {
      try {
        if (action === "list") {
          if (!parent_item_key) return fail(new Error("parent_item_key is required for list action"));
          const attachments = await zot.getItemAttachments(parent_item_key);
          if (!attachments.length) return ok(`No attachments found for item [${parent_item_key}].`);
          const lines = [`# Attachments for [${parent_item_key}]`, ""];
          for (const att of attachments) {
            lines.push(`- [${att.key}] ${att.title}`);
            if (att.filename) lines.push(`  Filename: ${att.filename}`);
            if (att.contentType) lines.push(`  Content type: ${att.contentType}`);
            if (att.path) lines.push(`  Path: ${att.path}`);
          }
          return ok(lines.join("\n"));
        }

        if (action === "import_file" || action === "link_file") {
          if (!parent_item_key) return fail(new Error("parent_item_key is required for file attachment actions"));
          ...
          const key = action === "import_file"
            ? await zot.uploadAttachment(parent_item_key, file_path, finalContentType, finalTitle)
            : await zot.linkLocalFileAttachment(parent_item_key, file_path, finalContentType, finalTitle);
          return ok(
            `${action === "import_file" ? "Imported" : "Linked"} attachment **${finalTitle}** [${key}] ` +
              `for item [${parent_item_key}]`
          );
        }

        if (action === "link_url") {
          if (!parent_item_key) return fail(new Error("parent_item_key is required for link_url action"));
          ...
          const key = await zot.createLinkedUrlAttachment(
            ...
            url,
            finalTitle,
            content_type || guessContentType(url, "application/pdf")
          );
          return ok(`Linked URL attachment **${finalTitle}** [${key}] for item [${parent_item_key}]`);
        }

        if (action === "update") {
          if (!attachment_key) return fail(new Error("attachment_key is required for update action"));
          if (title === undefined && tags === undefined) {
            return fail(new Error("Provide title or tags for update action"));
          }
