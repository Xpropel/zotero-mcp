...
export function registerTagTools(server: McpServer): void {
  server.tool(
    "zotero_tags",
    "List tags, batch add/remove tags, or rename/merge/delete a tag across local Zotero items.",
    {
      action: z.enum(["list", "batch", "rename", "merge", "delete"]).default("list").describe("Action to perform"),
      query: z.string().optional().describe("For batch: search query to find items"),
      add_tags: z.array(z.string()).optional().describe("For batch: tags to add"),
      remove_tags: z.array(z.string()).optional().describe("For batch: tags to remove"),
      ...
      confirm: z.boolean().default(false).describe("Required true for rename/merge/delete"),
    },
    async ({ action, query, add_tags, remove_tags, old_tag, new_tag, limit, confirm }) => {
      try {
        if (action === "list") {
        ...
        if (action === "batch") {
          if (!query) return fail(new Error("query is required for batch action"));
